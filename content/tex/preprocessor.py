...
def addref(caption, outstream):
    ...
    print(r"\cpbookref{%s}" % caption, file=outstream)
...
def processwithcomments(caption, instream, outstream, listingslang):
    ...
    out = []
    if warning:
        out.append(r"\cpbookwarning{%s: %s}" % (caption, warning))
    if error:
        out.append(r"\cpbookerror{%s: %s}" % (caption, error))
    else:
...
def processraw(caption, instream, outstream, listingslang = 'raw'):
    try:
        ...
        print("\cpbookerror{Could not read source.}", file=outstream)
