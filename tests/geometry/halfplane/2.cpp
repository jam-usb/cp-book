// https://codeforces.com/gym/101309 (problem J)
...
bool check(int d) {
    ...
    FOR(i,0,n) {
        int j = (i+d) % n;
        hplanes.pb({poly[i], poly[j]});
    }

    auto isection = halfplaneInters(hplanes);

    return areaPoly(isection) > eps;
}
